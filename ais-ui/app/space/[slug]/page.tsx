...
import prisma, { SpaceWithAll } from '@/lib/db-prisma';


async function getSpaceById(slug: string): Promise<SpaceWithAll> {
	try {
...
export default async function SpacePage(
	...
	const space: SpaceWithAll = await getSpaceById(params.slug);
	return (
		<>
			<div className="flex flex-col gap-2 items-start">
				...
				{space.SpaceSearch.map((search, index) => (
					<div key={search.id} className="max-w-md overflow-wrap">
						<Link color="primary"
							href={`/space/${space.slug}/search?id=${search.id}`}
							className={buttonVariants({ variant: "outline" })}>
							{search.llm_keywords?.replaceAll('|', ', ').slice(0, 99)}
						</Link>
					</div>
				))}
